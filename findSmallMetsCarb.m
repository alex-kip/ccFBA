function [metInfo] = findSmallMetsCarb(model,NoFormulaStr,cut_off)
...
% a capital X in the metFormula (e.g. 'HX')
 InorganicMetID=InorganicMetID(find(cellfun(@isempty,...
     regexp(model.metFormulas(InorganicMetID),'R'))),1);

OrganicMetID=setdiff(model.metIDs,InorganicMetID);
OrganicRxns=findRxnsFromMets(model,model.mets(OrganicMetID));
OrganicRxnID=findRxnIDs(model,OrganicRxns);

PercOrganicMet = length(OrganicMetID)/length(model.metIDs)*100;
PercOrganicRxn = length(OrganicRxnID)/length(model.rxnID)*100;
...
metInfo.organicMetIDs = OrganicMetID;
metInfo.organicrxnIDs = OrganicRxnID;
metInfo.PercOrganicMet = PercOrganicMet;
metInfo.PercOrganicRxn = PercOrganicRxn;
