function cofactorPairs = findCofactorPairs(model,metPairs)
 
pattern = {'nadh';'nadph';'atp';'gtp';'fadh2'};
% pattern = {'nadh';'nadph';'atp';'fadh2';'fmn';'thmpp';'ctp';'thmtp';'ribflv';'pydx5p';'gtp';'ttp';'utp'};
...
if exist('metPairs','var')
    if isempty(metPairs)
        ...
        metPairs = countMetPairs(model,smallMets.smallMetIDs,true,'_');
    end
else
    smallMets = findSmallMetsCarb(model);
    metPairs = countMetPairs(model,smallMets.smallMetIDs,true,'_');
...
if ~ismember('E',fnames)
    [E,e] = constructElementalMatrixNew(model.metFormulas);
    E = full(E);
else
